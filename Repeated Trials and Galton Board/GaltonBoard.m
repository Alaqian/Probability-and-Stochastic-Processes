...
classdef GaltonBoard < matlab.apps.AppBase
    ...
    methods (Access = private)
        ...
        function drop(app)
            ...
            %culx is used to record the final position of each ball
            culx = zeros(5);
            for i=1:numofballs
                x=2.5;
                y=6;
                %wait for each ball to fall to the bottom
                while y>0
                    ...
                    if y>5
                        y = y - velofballs;
                        % control the accuracy, to avoid a ball cannot
                ...
                % Wait for each ball finishs falling, clear fig 1.
                cla(app.UIAxes);
                if fix(x)>=0
                    if fix(x)==5
                        culx(5)=culx(5)+1;
        ...
        function results = probadir(app)
            pro=rand();
            % 50% probability goes to left
            if pro<=0.5
                results=1;
            % 50% probability goes to right
            else
                results=-1;
            end
        end
    ...
    methods (Access = private)
        ...
        function startupFcn(app)
            printfixedpoints(app)
            % When start, Pause function can not be used 
        ...
        function PauseButtonValueChanged(app, event)
            % change between pause and continue
